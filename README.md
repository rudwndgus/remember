...
**팰팍행 다듬기:** 이동 차량과 도착 차량은 주차선이 들어간 여백을 제외한 프레임을 사용합니다. 차 근처에서는 캐릭터 머리 위에 `?`가 표시됩니다. 동행 시작 시 엘리베이터와 차가 함께 보이도록 카메라를 축소하고, 동행자 보행·차량 출발을 빠르게 조정했습니다. 차량 내부 이동은 12초에서 6초로 단축했습니다. 승애는 스프라이트와 차량 내부 그림 모두 남색 옷·긴 머리로 통일했습니다. 내부 그림은 기존 `public/assets/car/shane-interior.png`를 built-in imagegen으로 편집했으며, 프롬프트는 조수석 여성의 옷과 머리만 변경하고 기존 구도·픽셀 스타일을 유지하도록 지정했습니다.
