...
**이미지 준비가 필요합니다.** 현재 저장소에는 대화에 첨부한 원본 로고와 지도 파일이 포함되어 있지 않습니다. 아래 `이미지` 항목의 두 경로에 원본 PNG를 저장한 뒤 실행하세요. 파일이 없으면 게임은 필요한 파일 경로와 다시 시도 버튼을 표시합니다.
...
원본 로고와 지도를 다음 경로에 직접 저장하세요. `public` 아래의 파일은 Vite가 빌드에 복사하고, 게임에서는 배포 경로를 적용한 URL로 읽습니다. PWA 아이콘은 프로젝트에 포함됩니다.
...
실제 기기에서는 원본 파일을 추가한 뒤 첫 실행 → 로고 원형 확대 → 지도 공개 → 전체 지도 유지 → 플레이어 확대 순서와 터치 조작을 확인하세요. 오프라인 검증은 `build`/`preview` 또는 HTTPS 배포에서 첫 로딩이 끝난 다음 네트워크를 끄고 새로고침합니다. PWA 설치 메뉴는 기기와 브라우저에 따라 달라집니다.
