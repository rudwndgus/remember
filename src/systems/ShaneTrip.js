...
export default class ShaneTrip {
  constructor(scene) {
    ...
    this.marker = scene.add.text(0, 0, '?', { fontFamily: 'monospace', fontSize: '18px', color: '#6b4826', backgroundColor: '#fff2be', padding: { x: 6, y: 4 } }).setOrigin(.5,1).setDepth(5).setVisible(false);
  ...
  update(delta) {
    ...
    this.marker.setPosition(this.scene.player.x, this.scene.player.y - this.scene.player.displayHeight - 8).setVisible(available);
  ...
  makeNPC(name, x, y, shirt, longHair) {
    ...
    if (!s.textures.exists(key)) {
      ...
      r(0x23332d,3,2,10,10); r(0x342a28,3,2,10,longHair?20:6);
      ...
      if(longHair) { r(0x342a28,3,6,2,15); r(0x342a28,11,6,2,15); }
  ...
  async begin() {
    ...
    await new Promise(resolve => s.tweens.add({targets:panel,scaleX:.05,duration:400,ease:'Sine.easeInOut',onComplete:resolve}));
    const shane=this.makeNPC('Shane',646,448,0x354d67,false);
    const seungae=this.makeNPC('승애',649,452,0x263c60,true);
    ...
    const b=pause(400).then(()=>this.move(seungae,[[651,500],[740,525],[740,770],[268,770],[268,638],[193,638]]));
    const playerPath=this.move(s.player,[[Math.max(219,s.player.x),s.player.y],[232,660],[188,660]],300);
    ...
    if(!texture.has('shane-car')) texture.add('shane-car',0,57,542,134,84);
    s.add.tileSprite(57,542,134,84,'parking-garage-map','asphalt').setOrigin(0);
    const car=s.add.image(124,584,'parking-garage-map','shane-car').setDepth(4);
    this.car = car;
    s.camera.startFollow(car,true,.08,.08);
    await this.move(car,[[270,584]],320);
    await new Promise(resolve=>s.tweens.add({targets:car,angle:90,duration:400,onComplete:resolve}));
    await this.move(car,[[270,800]],350);
    await new Promise(resolve=>s.tweens.add({targets:car,angle:0,duration:400,onComplete:resolve}));
    await this.move(car,[[724,800]],370);
    await new Promise(resolve=>s.tweens.add({targets:car,angle:90,duration:400,onComplete:resolve}));
    await this.move(car,[[724,1140]],380);
    s.scene.start('PalparkTripScene');
  }
  frameGathering(animate = true) {
    const s = this.scene, camera = s.camera;
    s.arrivalAnchor = null; s.arrivalSettling = false;
    camera.stopFollow(); camera.removeBounds();
    camera.panEffect.reset(); camera.off('camerapancomplete');
    const zoom = Math.min(s.scale.width / 960, s.scale.height / 660);
    if (!animate) { camera.setZoom(zoom); camera.centerOn(480, 560); return; }
    camera.pan(480, 560, 600, 'Sine.easeInOut');
    return new Promise(resolve => camera.zoomTo(zoom, 600, 'Sine.easeInOut', true,
      (_camera, progress) => { if (progress === 1) resolve(); }));
  }
  destroy() {
